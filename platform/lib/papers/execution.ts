...
const E4_1 = mk({
  ...
  reqEn: [
    ...
    "Revenue recognition carries a presumed fraud risk unless that presumption was rebutted in P5.1 (ISA 240 ¶26). Cut-off and occurrence are the assertions most often affected.",
  ],
  reqFr: [
    ...
    "La comptabilisation des produits porte une présomption de risque de fraude sauf réfutation en P5.1 (ISA 240 ¶26).",
  ],
  procs: [
    P("flow", "Record the critical path: how a sale is initiated, recorded, processed and reported, naming the actors, the documents, the applications and the control points.", "Consigner le chemin critique : initiation, enregistrement, traitement et restitution d'une vente, en nommant les acteurs, documents, applications et points de contrôle.", "Process narrative · inquiry of the cycle owner · P4.3", "Descriptif de processus · entretien avec le responsable du cycle · P4.3"),
    ...
    P("controls", "Where a controls-reliance strategy was set in S3.1, test the operating effectiveness of the identified controls over the period of reliance.", "Lorsque S3.1 retient l'appui sur les contrôles, tester l'efficacité du fonctionnement des contrôles identifiés sur la période concernée.", "S3.1 · control evidence · E1.2 for IT-dependent controls", "S3.1 · preuves de contrôle · E1.2 pour les contrôles dépendants de l'informatique"),
  ...
  items: [
    ...
    Q("fraud", "Nothing found indicates the fraud risk in revenue recognition identified in P5.1 has materialised (procedures 5, 6, 8).", "Rien ne montre que le risque de fraude sur les produits identifié en P5.1 s'est matérialisé (procédures 5, 6, 8)."),
  ],
  conclEn: [
    "Sufficient appropriate audit evidence has been obtained over the revenue and receivables flow for the assertions identified as relevant in S3.1.",
  ],
  conclFr: [
    "Des éléments probants suffisants et appropriés ont été obtenus sur le flux ventes et créances pour les assertions retenues en S3.1.",
  ],
});
...
const E4_2 = mk({
  ...
  procs: [
    ...
    P("controls", "Where a controls-reliance strategy was set in S3.1, test the operating effectiveness of the identified controls, including the three-way match and payment authorisation.", "Lorsque S3.1 retient l'appui sur les contrôles, tester leur efficacité, dont le rapprochement à trois documents et l'autorisation des paiements.", "S3.1 · control evidence · delegation of authority", "S3.1 · preuves de contrôle · délégations de pouvoirs"),
  ...
  conclEn: [
    "Sufficient appropriate audit evidence has been obtained over the purchases and payables flow for the assertions identified as relevant in S3.1, including completeness.",
  ],
  conclFr: [
    "Des éléments probants suffisants et appropriés ont été obtenus sur le flux achats et fournisseurs pour les assertions retenues en S3.1, dont l'exhaustivité.",
  ],
});
...
const E4_3 = mk({
  ...
  conclEn: [
    "Sufficient appropriate audit evidence has been obtained over payroll and personnel costs for the assertions identified as relevant in S3.1.",
  ],
  conclFr: [
...
const E1_2 = mk({
  std: "ISA 330 ¶8–17 · ISA 530 · ISA 500 ¶9",
  ownsEn: "the tests of controls over the significant classes of transactions — selection, sampling, execution, exceptions and the operating-effectiveness conclusion per control",
  ownsFr: "les tests des contrôles sur les flux significatifs — sélection, échantillonnage, exécution, exceptions et conclusion d'efficacité par contrôle",
  reqEn: [
    "A controls-reliance strategy carries the hypothesis that there are no or very few control exceptions in the population; tests of controls confirm that hypothesis by discovery sampling, and any exception found calls the reliance itself into question.",
    "Inquiry alone never provides sufficient evidence of operating effectiveness — it is combined with inspection, observation, reperformance or data analysis.",
    "Minimum sample sizes follow the frequency of the control: daily/many-times 25 (60 when it is the only control on its assertion), weekly 5, monthly and quarterly 2, annually 1; 50–250 occurrences → 10%, under 50 → 5, under 5 → all; an automated control supported by effective ITGCs takes a test of one.",
    "Tests are performed on every sampling unit selected; an item that cannot be tested and has no valid replacement is a control exception, never a skip.",
  ],
  reqFr: [
    "Une stratégie d'appui sur les contrôles repose sur l'hypothèse d'exceptions rares ou nulles ; les tests la confirment par sondage de découverte, et toute exception remet l'appui en cause.",
    "L'entretien seul ne suffit jamais — il se combine avec l'inspection, l'observation, la réexécution ou l'analyse de données.",
    "Tailles minimales selon la fréquence : quotidien 25 (60 si contrôle unique sur son assertion), hebdomadaire 5, mensuel et trimestriel 2, annuel 1 ; 50–250 occurrences → 10 %, moins de 50 → 5, moins de 5 → 100 % ; contrôle automatisé soutenu par des ITGC efficaces → test unique.",
    "Chaque unité d'échantillonnage sélectionnée est testée ; un élément intestable sans remplacement valable est une exception, jamais une omission.",
  ],
  procs: [
    P("selection", "Confirm from S2.2 that the controls selected for testing address each WCGW for each relevant assertion of the SCOT, preferring controls that answer several WCGWs; only selected relevant controls appear on the board below.", "Confirmer à partir de S2.2 que les contrôles retenus couvrent chaque WCGW pour chaque assertion pertinente du SCOT, en privilégiant les contrôles couvrant plusieurs WCGW ; seuls les contrôles retenus figurent au tableau ci-dessous.", "S2.2 design · WCGW–control links · S3.1 strategy", "Conception S2.2 · liens WCGW–contrôles · stratégie S3.1"),
    P("changes", "Before executing, determine whether any control changed since the design was confirmed (application, policies, IT, personnel, circumstances); understand, re-document and re-walkthrough any change and adjust the planned tests.", "Avant l'exécution, déterminer si un contrôle a changé depuis la conception (application, procédures, informatique, personnel, circonstances) ; comprendre, redocumenter, refaire le cheminement et ajuster les tests prévus.", "Inquiries · updated narratives · walkthrough", "Entretiens · descriptifs mis à jour · cheminement"),
    P("nature", "Design each test as inquiry COMBINED with inspection, observation, reperformance or data analysis; document the control attributes whose presence the test establishes.", "Concevoir chaque test comme un entretien COMBINÉ avec inspection, observation, réexécution ou analyse de données ; documenter les attributs du contrôle que le test établit.", "Test design per control · attribute list", "Conception par contrôle · liste d'attributs"),
    P("population", "Define, per control, the complete population of occurrences for the period of reliance and record it on the board — the minimum sample size follows automatically; select without bias (random, systematic or haphazard).", "Définir, par contrôle, la population complète des occurrences sur la période d'appui et la saisir au tableau — la taille minimale en découle automatiquement ; sélection sans biais (aléatoire, systématique ou empirique).", "Population evidence · completeness check · board", "Preuve de population · exhaustivité · tableau"),
    P("execute", "Execute on every sampling unit: record each transaction tested in the control's grid, marking every attribute passed, failed or n/a; use replacement items only where the selected item was genuinely not subject to the control.", "Exécuter sur chaque unité : consigner chaque transaction testée dans la grille du contrôle, chaque attribut réussi, en échec ou n/a ; remplacer un élément uniquement s'il n'était réellement pas soumis au contrôle.", "Transactions-tested grids · supporting documents", "Grilles de transactions testées · pièces justificatives"),
    P("automated", "For automated and IT-dependent controls, confirm the E1.1 ITGC conclusion supports each application for the whole period (then a test of one suffices), and test the accuracy and completeness of any report the control or the test relies on.", "Pour les contrôles automatisés et dépendants de l'informatique, confirmer que la conclusion E1.1 soutient chaque application sur toute la période (un test unique suffit alors), et tester l'exactitude et l'exhaustivité des états utilisés.", "E1.1 conclusion · configuration · IPE testing", "Conclusion E1.1 · paramétrage · tests des états"),
    P("exceptions", "For every exception: establish the cause and implication, decide extend-or-stop, evaluate the deficiency (significant deficiency where warranted), and conclude the control Effective or Not effective on the board; where not effective, revise S3.1 to not-rely and extend the substantive response.", "Pour chaque exception : établir la cause et l'incidence, décider d'étendre ou d'arrêter, évaluer la déficience, et conclure Efficace ou Non efficace au tableau ; si non efficace, réviser S3.1 (pas d'appui) et étendre la réponse substantive.", "Exception analysis · deficiency evaluation · S3.1", "Analyse des exceptions · évaluation de déficience · S3.1"),
    P("rotation", "Where reliance rests on prior-period evidence for a routine SCOT, confirm every rotation criterion: effective when last tested, control environment sound, understanding reconfirmed, no relevant change, tested at least once in every third audit, and at least one routine SCOT tested this period; never rotate significant-risk, non-routine, estimation or bank-reconciliation controls.", "Si l'appui repose sur des preuves antérieures pour un SCOT routinier, confirmer chaque critère de rotation : efficace au dernier test, environnement de contrôle solide, compréhension reconfirmée, aucun changement pertinent, test au moins tous les trois audits, et au moins un SCOT routinier testé cette période ; jamais de rotation pour risques importants, flux non routiniers, estimations ou rapprochements bancaires.", "Prior files · rotation schedule · change inquiry", "Dossiers antérieurs · calendrier de rotation · entretiens"),
  ],
  items: [
    Q("selection", "The controls tested address each WCGW for each relevant assertion, per the S2.2 design (procedure 1).", "Les contrôles testés couvrent chaque WCGW pour chaque assertion pertinente, selon la conception S2.2 (procédure 1)."),
    Q("changes", "No control changed since design — or every change was understood, re-documented, re-walked-through and answered (procedure 2).", "Aucun contrôle n'a changé depuis la conception — ou chaque changement a été compris, redocumenté, recheminé et traité (procédure 2)."),
    Q("nature", "No conclusion rests on inquiry alone (procedure 3).", "Aucune conclusion ne repose sur le seul entretien (procédure 3)."),
    Q("population", "Every population was complete and appropriate, and every sample met the minimum size, selected without bias (procedure 4).", "Chaque population était complète et appropriée, chaque échantillon a atteint la taille minimale, sélection sans biais (procédure 4)."),
    Q("grids", "Every transaction tested is recorded in the control's grid with each attribute concluded (procedure 5).", "Chaque transaction testée figure dans la grille du contrôle avec chaque attribut conclu (procédure 5)."),
    Q("automated", "For each automated or IT-dependent control, ITGC support was confirmed and the reports used were tested (procedure 6).", "Pour chaque contrôle automatisé ou dépendant de l'informatique, le soutien des ITGC a été confirmé et les états utilisés testés (procédure 6).", true),
    Q("exceptions", "Every exception was traced to cause and implication, evaluated as a deficiency, and reflected in the control's conclusion (procedure 7).", "Chaque exception a été tracée (cause, incidence), évaluée en déficience et reflétée dans la conclusion du contrôle (procédure 7).", true),
    Q("rotation", "For any rotated routine SCOT, every rotation criterion is met and documented (procedure 8).", "Pour tout SCOT routinier en rotation, chaque critère est satisfait et documenté (procédure 8).", true),
  ],
  conclEn: [
    "The controls selected for testing over the significant classes of transactions operated effectively throughout the period of reliance; every exception was evaluated and answered; and the reliance recorded in S3.1 stands — or has been revised to not-rely with the substantive response extended.",
  ],
  conclFr: [
    "Les contrôles retenus sur les flux significatifs ont fonctionné efficacement sur la période d'appui ; chaque exception a été évaluée et traitée ; l'appui consigné en S3.1 est maintenu — ou révisé vers l'absence d'appui avec extension de la réponse substantive.",
  ],
});
...
const E4_5 = mk({
  ...
  conclEn: [
    "Sufficient appropriate audit evidence has been obtained over property, plant and equipment for the assertions identified as relevant in S3.1.",
  ],
  conclFr: [
    "Des éléments probants suffisants et appropriés ont été obtenus sur les immobilisations corporelles pour les assertions retenues en S3.1.",
  ],
});
...
const E4_7 = mk({
  ...
  procs: [
    ...
    P("valuation", "Test the valuation: agree the quoted holdings to the market price at the year end, and test the basis of any unquoted valuation.", "Tester la valorisation : rapprocher les titres cotés du cours de clôture et tester la base de toute valorisation non cotée.", "Market data at the year end · valuation model · E6.7", "Cours de clôture · modèle de valorisation · E6.7"),
  ...
  conclEn: [
    "Sufficient appropriate audit evidence has been obtained over investments and financial assets for the assertions identified as relevant in S3.1.",
  ],
  conclFr: [
    "Des éléments probants suffisants et appropriés ont été obtenus sur les titres et actifs financiers pour les assertions retenues en S3.1.",
  ],
});
...
const E4_10 = mk({
  ...
  procs: [
    ...
    P("filings", "Confirm that every return due in the period was filed, and identify any filed late together with the penalty arising.", "Vérifier le dépôt de chaque déclaration due sur l'exercice et identifier les dépôts tardifs et les pénalités correspondantes.", "Filing receipts · tax account statement · C5.2", "Récépissés de dépôt · état de compte fiscal · C5.2"),
  ],
  items: [
  ...
  conclEn: [
    "Sufficient appropriate audit evidence has been obtained over value added tax and other sales taxes, and any non-compliance identified has been carried to E6.1.",
  ],
  conclFr: [
    "Des éléments probants suffisants et appropriés ont été obtenus sur la TVA et taxes assimilées, et toute non-conformité relevée est reportée en E6.1.",
  ],
});
...
const E4_13 = mk({
  ...
  procs: [
    ...
    P("reverse", "Review the operating result for items that should have been classified as HAO but were not.", "Examiner le résultat d'exploitation à la recherche d'éléments qui auraient dû être classés en HAO.", "General ledger · analytical review P3.2", "Grand livre · revue analytique P3.2"),
...
const E4_16 = mk({
  ...
  procs: [
    ...
    P("register", "Agree the shareholdings to the register of registered securities, and cross-refer to C5.7.", "Rapprocher la répartition du capital du registre des titres nominatifs et renvoyer à C5.7.", "Share register · C5.7 · statutes", "Registre des titres · C5.7 · statuts"),
    ...
    P("half_capital", "Compare net equity with half of the share capital, and cross-refer to C5.8 where the threshold is breached.", "Comparer les capitaux propres à la moitié du capital social et renvoyer à C5.8 en cas de franchissement du seuil.", "Trial balance · statutes · C5.8", "Balance · statuts · C5.8"),
  ],
  items: [
...
const E6_1 = mk({
  ...
  procs: [
    P("framework", "Establish the legal and regulatory framework applicable to the entity and its sector, and identify the texts with a direct effect on the financial statements.", "Établir le cadre légal et réglementaire applicable à l'entité et à son secteur et identifier les textes ayant une incidence directe sur les comptes.", "Statutes · sector regulation · OHADA Uniform Acts · P3.1", "Statuts · réglementation sectorielle · Actes uniformes OHADA · P3.1"),
...
const E6_2 = mk({
  ...
  procs: [
    P("register", "Bring forward the related party register from S4.3 and confirm it remains complete at the year end.", "Reprendre le registre des parties liées de S4.3 et confirmer son exhaustivité à la clôture.", "S4.3 · inquiry of management", "S4.3 · entretien avec la direction"),
    P("search", "Search the ledger, the bank statements and the minutes for transactions with the parties on the register, and for names not on it.", "Rechercher dans le grand livre, les relevés bancaires et les procès-verbaux les opérations avec les parties du registre et les noms absents de celui-ci.", "General ledger · bank statements · minutes (E6.4)", "Grand livre · relevés bancaires · procès-verbaux (E6.4)"),
    ...
    P("disclosure", "Test the related party disclosure in the draft financial statements against the register and the transactions tested, and against the conventions réglementées in C5.3.", "Tester l'information sur les parties liées du projet d'états financiers au regard du registre, des opérations testées et des conventions réglementées de C5.3.", "Draft financial statements · C5.3 · framework", "Projet d'états financiers · C5.3 · référentiel"),
  ],
  items: [
...
const E6_3 = mk({
  ...
  procs: [
    P("carry", "Bring forward the events and conditions identified in S4.2 and confirm the position on each.", "Reprendre les événements et conditions identifiés en S4.2 et confirmer leur situation.", "S4.2 · inquiry of management", "S4.2 · entretien avec la direction"),
  ...
  conclEn: [
    ...
    "The effect of our conclusion on the auditor's report has been determined and recorded in C5.1.",
  ],
  conclFr: [
    "L'application du principe de continuité d'exploitation est appropriée.",
    "L'effet de notre conclusion sur le rapport a été déterminé et consigné en C5.1.",
  ],
});
...
const E6_4 = mk({
  ...
  procs: [
    ...
    P("registers", "Inspect the statutory registers: the register of registered securities, the register of attendance and the register of the board's deliberations.", "Examiner les registres statutaires : registre des titres nominatifs, feuille de présence et registre des délibérations.", "Statutory registers · C5.7", "Registres statutaires · C5.7"),
    P("filings", "Confirm that the prior period accounts were approved within the statutory deadline and filed with the RCCM.", "Vérifier que les comptes de l'exercice précédent ont été approuvés dans le délai légal et déposés au RCCM.", "AGO minutes · RCCM filing receipt · C5.2", "Procès-verbal d'AGO · récépissé de dépôt RCCM · C5.2"),
    P("subsequent", "Read the minutes of the meetings held after the year end for events requiring adjustment or disclosure, and carry each to E6.6.", "Examiner les procès-verbaux des réunions postérieures à la clôture à la recherche d'événements à ajuster ou à mentionner et les reporter en E6.6.", "Post year-end minutes · E6.6", "Procès-verbaux postérieurs · E6.6"),
  ],
  items: [
...
const E6_6 = mk({
  ...
  reqEn: [
    ...
    "The procedures cover the whole period up to the date of the report. C2.2 records the final position at completion; this paper records the work performed during execution.",
  ],
  reqFr: [
    ...
    "Les procédures couvrent la période jusqu'à la date du rapport. C2.2 consigne la position finale à l'achèvement.",
  ],
  procs: [
    ...
    P("minutes", "Read the minutes of the meetings held after the date of the financial statements.", "Examiner les procès-verbaux des réunions tenues après la date des états financiers.", "Post year-end minutes (E6.4)", "Procès-verbaux postérieurs (E6.4)"),
  ...
  items: [
    ...
    Q("carried", "Each event identified has been carried to C2.2 for the position at the date of the report (procedure 6).", "Chaque événement identifié est reporté en C2.2 pour la position à la date du rapport (procédure 6)."),
  ],
  conclEn: [
    "The subsequent events procedures performed during execution identified every event requiring adjustment or disclosure up to the date of this work, and the position is carried to C2.2.",
  ],
  conclFr: [
    "Les procédures sur les événements postérieurs mises en œuvre en exécution ont identifié tout événement à ajuster ou à mentionner jusqu'à la date de ces travaux, et la position est reportée en C2.2.",
  ],
});
