...
//                         sign-off on the D3.1 working paper, independence
//                         confirmations complete with exceptions disposed.
...
/** Active (non-voided) partner sign-off on the working paper of a file-index code. */
...
export async function acceptanceGates(engagementId: string): Promise<GateResult[]> {
  ...
  return withTenant(tenantId, async (tx) => {
    ...
    const independence = await tx.query<{ open: string; undisposed: string }>(
      `SELECT
    ...
    return [
      ...
      { key: "independence_complete", ok: Number(indep.open) === 0 },
...
/** acceptance → planning (blocks on acceptance gates). */
export async function advanceToPlanning(engagementId: string): Promise<void> {
  ...
  await withTenant(tenantId, async (tx) => {
    await tx.query(
      "UPDATE engagement SET phase = 'planning' WHERE id = $1 AND phase = 'acceptance'",
      [engagementId],
    );
  });
}

/** planning → execution: blocks on close gates, then snapshots the plan (spec §5.4). */
export async function closePlanning(engagementId: string): Promise<void> {
  ...
  await withTenant(tenantId, async (tx) => {
    ...
    await tx.query(
      "UPDATE engagement SET phase = 'execution' WHERE id = $1 AND phase = 'planning'",
      [engagementId],
    );
  });
}
