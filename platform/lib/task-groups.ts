...
export const TASK_GROUPS: TaskGroupDef[] = [
  ...
  // ---- Execution ----
  // E5 is the home of the general audit procedures: journal entries, estimates,
  // related parties, going concern, laws and regulations, litigation and claims,
  // and subsequent events, in the order the work is performed. The members keep
  // the codes they were stored under (E3.1, E6.x) because grouping is not held
  // in the database — the display code comes from the member's position below,
  // so a task can be rehomed without touching file_item.code and without
  // stranding a single form_response answer.
  //
  // Journal entry testing was the whole of the old E3 group, so that group is
  // gone rather than left empty; the execution phase now runs E1, E2, E4, E5, E6.
  //
  // The five P&L lead schedules (Operating Expenditures through Other Income)
  // are duplicated by the E4.20–E4.33 account papers, so they leave the general
  // procedures. They sit at the end of E4 instead of nowhere: a task in no group
  // falls out of the phase and group screens altogether, and would keep its raw
  // "E5.1" label, which is the display code E3.1 now answers to.
