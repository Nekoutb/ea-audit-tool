...
describe("working-paper docx generation", () => {
  it("resolves the P1.1 template and falls back to generic elsewhere", () => {
    expect(templateFor("P1.1").id).toBe("P1.1-acceptance");
    expect(templateFor("E4.1").id).toBe("generic");
  });

  it("renders a valid .docx (ZIP container) in both languages", async () => {
    for (const locale of ["en", "fr"] as const) {
      const buffer = await generateWorkpaperDocx(templateFor("P1.1"), FIELDS, locale);
